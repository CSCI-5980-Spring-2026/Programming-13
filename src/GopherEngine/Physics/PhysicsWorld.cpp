...
namespace GopherEngine
{
    PhysicsWorld::PhysicsWorld()
    {
        ...
        dynamics_world_->setGravity(btVector3(0.f, -9.81f, 0.f));
    }
    ...
    void PhysicsWorld::update(float delta_time)
    {
        ...
        for (auto& [collider_id, entry] : colliders_)
        {
            ...
            if (entry.collider_component_ == nullptr || entry.owner_node_ == nullptr || entry.object_ == nullptr)
                continue;

            // Keep Bullet aligned with the latest scene-authored world transform,
            // even for rigid bodies that are being teleported by game logic.
    ...
    ColliderId PhysicsWorld::register_rigid_body(RigidBodyComponent* component, Node& node)
    {
        ...
        // If the collider has not been registered yet, there is nothing Bullet
        // can simulate. We leave the entry in a pending state and let the later
        // collider registration finish the Bullet-side object creation.
        if (entry.collider_component_ == nullptr || entry.shape_ == nullptr)
            return collider_id;
    ...
    void PhysicsWorld::unregister_rigid_body(ColliderId collider_id)
    {
        ...
        // Clear only the rigid-body-owned state. If a collider still exists, the
        // node should fall back to a plain collision object rather than vanish
        // from the physics world entirely.
        entry.rigid_body_component_ = nullptr;
